...
// .wrangler/tmp/bundle-vktUSI/checked-fetch.js
...
// .wrangler/tmp/bundle-vktUSI/strip-cf-connecting-ip-header.js
...
// src/telemetrySchema.ts
var TELEMETRY_INDEX_FIELDS = ["aiDifficulty"];
var TELEMETRY_BLOB_FIELDS = [
  "anonId",
  "appVersion",
  "liveConfigVersion",
  "map",
...
function resolveValue(body, field) {
  const top = body[field];
  if (top !== void 0)
    return top;
  const player = body.player;
  return player ? player[field] : void 0;
}
__name(resolveValue, "resolveValue");
function asBlob(v) {
  return v === void 0 || v === null ? "" : String(v);
}
__name(asBlob, "asBlob");
function asDouble(v) {
  const n = Number(v);
  return Number.isFinite(n) ? n : 0;
}
__name(asDouble, "asDouble");
function toAnalyticsDataPoint(body) {
  return {
...
__name(toAnalyticsDataPoint, "toAnalyticsDataPoint");
...
var MAX_REPLAY_UPLOAD_BYTES = 64 * 1024 * 1024;
...
var src_default = {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/api/replay/list" && request.method === "GET") {
      ...
      const limit = Math.min(Number(url.searchParams.get("limit")) || 1e3, 1e4);
      try {
...
// .wrangler/tmp/bundle-vktUSI/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-vktUSI/middleware-loader.entry.ts

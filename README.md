...
Phase 7 added match-wide visual policy plumbing, loading-preview policy resolution, and replay visual metadata. Gameplay match setup currently exposes only V1 Classic and V2 Rigged while V3 remains locked out of player-selectable gameplay.
...
- **Model System Versions**: Select between different model systems:
  ...
  - **Version 2 (Rigged)**: High-fidelity skeletal bone & joint system with enhanced state-based animations (breathing idles, walking leg swing cycles with knees bending and ankle/toe flexes, sprinting leans and arm pumps, crouched sliding folds, and elbow-bending weapon swings).
    ...
    - **Hitbox Constraints & Customizer Safety**: To support future custom voxel parts (via a model maker/editor), V2 enforces a strict, standardized hitbox size constraint (in voxels) for each of the 15 body parts. Any custom part must fit within these bounding boxes:
      ...
      * `foot`: Max `6 x 8 x 5`
      * `toes`: Max `6 x 5 x 4`
  - **Version 3 (Advanced, Locked)**: A parallel voxel model system for modular armor, V3 weapons, first-person parity, layered procedural animation, and local V3 custom armor editing. V3 remains available to local editor/tooling paths, but it is not currently player-selectable for gameplay because the models are not ready for matches. Gameplay visual policy accepts Version 1 Classic and Version 2 Rigged; incoming V3 gameplay policies normalize to Version 2 while combat ranges, collision, hitboxes, weapon timing, movement physics, and AI decisions remain normalized.
    - **Match Visual Policy**: Match setup defaults to **Version 2 Rigged** and supports **Version 1 Classic** and **Version 2 Rigged** model policies. Offline sandbox applies the selected policy to the local player and bots without changing gameplay collision or weapon logic. Multiplayer hosts choose the lobby policy through `MatchLobbyConfig.visualModelPolicy`; clients and observers consume the normalized host value. Loading screens render participant previews through the active match policy. New replays store the normalized policy and sanitized visual loadout metadata, while older replays without policy continue to use legacy V1 playback visuals.

...
Tournament mode includes a **NeuralNet** bracket option that runs the exported `CombatDRV2` PPO browser brain for live 1v1 matches. The current browser artifact lives under `public/brains/combat_dr_v2/` (`manifest.json`, `weights.bin`, and `fixtures.json`) and is generated from `python/runs/combat_dr_v2/final_model.zip` with `python -m ibrawls_rl.export_browser_brain`. The manifest pins the trained contract: combat mode, observation version 1, env spec v4, frame stack 4, decision interval 5, input width 560, and action factors `[9, 4, 3, 2, 2, 2]`.
...
Phase 15 extends the high-density V3 armor-surface pass across the full built-in Aegis armor set. Every built-in V3 character part now uses gridScale 2 voxel payloads rendered at half voxel scale, with slot-specific shoulder caps, arm bands, bracers, gloves, pelvis plates, leg guards, boots, collar, backpack rails, and emissive details so the suit reads as segmented armor instead of a raw blockout. Player-authored V3 custom armor remains saved as voxel data, V1/V2 behavior remains unchanged, and gameplay collision, timing, scoring, AI, and network authority are still untouched.

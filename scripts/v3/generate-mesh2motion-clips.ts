...
import {
  generateV3Mesh2MotionArmorRigSourceFile,
  generateV3Mesh2MotionClipsSourceFile,
} from '../../src/tools/v3Mesh2MotionImporter';

const inputIndex = process.argv.indexOf('--input');
const outputIndex = process.argv.indexOf('--out');
const rigOutputIndex = process.argv.indexOf('--rig-out');
...
const rigOutputPath = rigOutputIndex >= 0
  ? process.argv[rigOutputIndex + 1]
  : 'src/components/v3/v3Mesh2MotionArmorRig.generated.ts';

if (!inputPath || !outputPath || !rigOutputPath) {
  throw new Error('Usage: npm run v3:generate-mesh2motion-clips -- --input <mesh2motion glb> --out <clips generated ts path> --rig-out <armor rig generated ts path>');
}
...
const armorRig = generateV3Mesh2MotionArmorRigSourceFile({
  filePath: resolve(inputPath),
  outputPath: resolve(rigOutputPath),
  fps: 30,
});
